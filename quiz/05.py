...
class Tab2(QWidget):
    def __init__(self, devs):
        ...
        self.form_layout.addRow('질문:', self.option2_line_edit)
        self.form_layout.addRow('질문:', self.option3_line_edit)
        self.form_layout.addRow('질문:', self.publish_clear_layout)
...
class Tab3(QWidget):
    ...
    def modify(self):
        ...
        self.devs.chain[-1]['transaction']['type']['quesiton'] = 'hack'
        self.devs.chain[-1]['transaction']['type']['options'] = ['hack1', 'hack2', 'hack3']
        self.devs.tab1.update_vote_list()


class SocketReceiver(QThread):
    update_vote_list_siganl = pyqtSignal()
    ...
    def run(self):
        while True:
            ...
            if data['transaction']['type'] == 'connect':
                ...
                self.devs.nodes.append((s, f'127.0.0.1:{data["trnasaction"]["data"]["port"]}'))
                ...
                self.update_vote_list_siganl.emit()
...
class SocketListener(QThread):
    ...
    def run(self):
        while True:
            ...
            self.receive_thread.update_vote_list_siganl.connect(self.update_vote_list)
...
class DecentralizedElectronicVotingSystem(QWidget):
    def __init__(self):
        ...
        self.tabs.addTab(self.tab1, 'hack')
