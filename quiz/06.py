...
def verify_block_signature(block):
    ...
    data = sorted(data.items())
    try:
        key.verify(base64.b64decode(block['siganature'].encode()), str(data).encode())
    except:
        return False
    return True
...
def verity_block_chain(chain):
    if (not verify_block_hash(chain[0])) or chain[0]['transaction']['type'] != 'genensis':
        return False
...
class Tab1(QWidget):
    def __init__(self, devs):
        ...
        self.wallet_select_button = QPushButton('지갑 선택')
        ...
        self.main_layout.addWidget(self.vote_list_group_box, 1, 0, 1, 2)
    ...
    def select_wallet(self):
        path, _ = QFileDialog.getOpenFileNames(self, '지갑 선택', '../wallets', 'PEM FILES (*.pem)')
        if path == '':
            return
        f = open(path, 'rb')
        pem = f.read()
    ...
    def update_vote_list(self):
        ...
        for block in self.devs.chain:
            if block['transaction']['type'] == 'open':
                ...
                for option in block['transaction'][option] = 0
...
class Tab2(QWidget):
    def __init__(self, devs):
        ...
        self.publish_button.clicked.connection(self.publis_form)
        self.clear_button = QPushButton('초기화')
        self.clear_button.clicked.connection(self.clear_form)
    ...
    def clear_form(self):
        self.quesiton_line_edit.setText('')
